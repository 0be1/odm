...
public class AttributeMetadataFactory<T> {
	...
	public AttributeMetadataFactory(final Class<T> persistantClass,
	...
	class Builder {
		...
		public AttributeMetadata build() throws MappingException {
			...
			if (objectType == null)
				throw new MappingException(String.format(
						"Could not determine type for attibute %s", name));
			...
			if (syntax == null)
				throw new MappingException(String.format(
						"No syntax specified  for attribute %s", name));
		...
		private Converter getSyntaxConverter(final String syntax)
		...
		private Converter getAttributeConverter(final Type objectType)
				throws MappingException {
			...
			if (retval == null
				...
				throw new MappingException(String.format(
						"No converter found for type %s", objectType));
			}
		...
		private final Type getObjectType(Class<?> c) {
			...
			if (Arrays.asList(c.getInterfaces()).contains(Collection.class)
				...
				if (genericFieldType instanceof ParameterizedType) {
					...
					if (fieldArgTypes.length > 1)
						throw new UnsupportedOperationException(
								"multivlaued attributes are persisted only as List or Set");
