...
public class EhCacheManager implements CacheManager {
	...
	public EhCacheManager(final SessionFactoryImpl sessionFactory,
	...
	private Ehcache getCache(final Class<?> persistentClass) {
		...
		final Ehcache cache = cacheManager
				.getEhcache(cacheName(persistentClass));

		if (cache != null) {
			return cache;
		}

		final Ehcache memoryOnlyCache = new net.sf.ehcache.Cache(name, 5000,
				false, false, 5, 2);
	...
	public <T> EntityCache<T> getCacheFor(Class<T> persistentClass) {
		final ClassMetadata<T> metaData = sessionFactory
