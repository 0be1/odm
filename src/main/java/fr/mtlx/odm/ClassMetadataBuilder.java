...
public class ClassMetadataBuilder<T> {
	...
	public PartialClassMetadata<T> build() throws MappingException {
		final PartialClassMetadata<T> metadata = new PartialClassMetadata<T>(
				persistentClass);

		if (Modifier.isAbstract(persistentClass.getModifiers()))
			throw new InstantiationError(String.format("%s is abstract",
		...
		metadata.setObjectClassHierarchy(ImmutableList.copyOf(Lists
				.reverse(classHierarchy)));
		...
		try {
			...
			throw new MappingException(String.format(
					"no default contructor found for %s", persistentClass), e);
		}
