...
class PropertyFilterBuilder<T, A> extends AttributeFilterBuilder<T> {
	...
	if (attributeType instanceof Class<?>) {
	    ...
	    ClassMetadata<A> refmetadata = sessionFactory.getClassMetadata(clazz);

	    if (refmetadata != null) {
		return encodeValue(refmetadata);
	    }
    ...
    @Override
    protected Encoder build(Comparison op) {
	return encoder;
    }

    private Encoder encodeValue(final ClassMetadata<A> attributeClassMetadata) {
