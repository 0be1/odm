...
public class PartialClassMetadata<T> implements ClassMetadata<T>
{
	...
	private Map<String, AttributeMetadata> attributeMetadataByAttributeName = new TreeMap<String, AttributeMetadata>( String.CASE_INSENSITIVE_ORDER );

	private Map<String, AttributeMetadata> attributeMetadataByPropertyName = new TreeMap<String, AttributeMetadata>( String.CASE_INSENSITIVE_ORDER );
	...
	@Override
	public AttributeMetadata getAttributeMetadataByAttributeName( final String attributeId )
	{
		return attributeMetadataByAttributeName.get( attributeId );
	}

	@Override
	public AttributeMetadata getAttributeMetadataByPropertyName( String propertyName )
	{
	...
	private void initPersistentAttributes( final SessionFactoryImpl sessionFactory )
	{
		final List<AttributeMetadata> attributes = Lists.newArrayList();

		ReflectionUtils.doWithFields( persistentClass, new FieldCallback()
		{
			public void doWith( @Nullable final Field f ) throws IllegalArgumentException, IllegalAccessException
			{
				...
				try
				{
					attributes.add( new AttributeMetadataFactory<T>( persistentClass, sessionFactory ).build( f ) );
				}
		...
		for ( final AttributeMetadata metadata : attributes )
		{
			...
			{
				assert attributeMetadataByPropertyName.containsKey( metadata.getPropertyName() );

				log.warn( "Attribute {} already declared.", metadata.getAttirbuteName() );
			}
			else
			{
				...
				attributeMetadataByPropertyName.put( metadata.getPropertyName(), metadata );
			}
	...
	public void setAttributeMetadataByAttributeName( Map<String, AttributeMetadata> attributeMetadataByAttributeName )
	{
	...
	public void setAttributeMetadataByPropertyName( Map<String, AttributeMetadata> attributeMetadataByPropertyName )
	{
	...
	public void setObjectClassHierarchy( ImmutableList<String> objectClassHierarchy )
	{
